...
export function DocumentVersionHistory({ type = 'both' }: DocumentVersionHistoryProps) {
  ...
  const [selectedType, setSelectedType] = useState<'all' | 'legal' | 'procedure'>('all');
  ...
  const filteredDocuments = documentOptions.filter(docTitle => {
    ...
    const matchesType = selectedType === 'all' || versions.some(v => v.type === selectedType);
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Button
              variant={selectedType === 'legal' ? 'default' : 'outline'}
              size="sm"
